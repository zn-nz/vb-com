<template>
	<div v-loading="loading" class="zn-table-content" :style="{ height }">
		<el-table ref="tableRef" class="flex-1" :class="{ 'hide-checkAll': hideDftCheckAll }" :data="data" @selection-change="handleSelectionChange" v-bind="{ ...prop }" v-on="{ ...propFuc }">
		...
		<div v-show="data?.length" class="flex al-center">
			<div v-if="(showSelect && hideDftCheckAll && !showBatchDel) || (showSelect && $slots.footer)" style="width: 55px" class="flex ju-center m-r-28 m-t-24">
				<el-checkbox :model-value="checkAll" :indeterminate="indeterminate" class="table-check-all" @change="handleCheckAll"></el-checkbox>
			</div>
			<div class="m-t-24">
